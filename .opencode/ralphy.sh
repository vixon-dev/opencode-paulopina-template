...
# Adapted for opencode-paulopina-template
#
# Supports: OpenCode (default), Claude Code, Codex, Cursor, Qwen-Code, Factory Droid
# Runs until PRD is complete or single task is done
# ============================================

set -euo pipefail

# ============================================
# OPENCODE TEMPLATE INTEGRATION
# ============================================
...
# Load project-scoped env if available (for NTFY_TOPIC, etc.)
...
# Notification helper (uses .opencode/notify.sh if available)
send_notification() {
  local type="$1"
  local message="$2"
  ...
  if [[ -x "$OPENCODE_DIR/notify.sh" ]]; then
    RALPHY_CONTEXT=1 "$OPENCODE_DIR/notify.sh" "$type" "$message" "$priority" "$tags" || true
  fi
}
...
SKIP_TESTS=false
SKIP_LINT=false
AI_ENGINE="opencode"  # opencode (default), claude, cursor, codex, qwen, or droid
DRY_RUN=false
...
# ============================================
# NOTIFICATION (Cross-platform + ntfy.sh integration)
# ============================================

notify_done() {
  local message="${1:-Ralphy has completed all tasks!}"
  
  # Use .opencode/notify.sh for ntfy.sh push notifications
  send_notification "done" "$message" "high" "heavy_check_mark,robot"
...
notify_error() {
  ...
  # Use .opencode/notify.sh for ntfy.sh push notifications
  send_notification "error" "$message" "urgent" "warning,robot"
